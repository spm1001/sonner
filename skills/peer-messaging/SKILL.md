...
- **sonnette** — the older account-keyed WebSocket mesh. **Delisted from the batterie marketplace 2026-08-24 (son-pilalu); do not reach for it.** Its cross-machine reach was real and nothing here replaces it — sonner discovers peers over unix sockets and is machine-local by construction — but that was accepted knowingly at the delisting: its measured traffic had stopped on 2026-07-19, five weeks earlier. **Cross-machine peer messaging is simply not a capability the estate has now.** If you need one host to reach another, use a file both can see, or the board. The code survives unpublished in `spm1001/aboyeur` (aby-fefife). Its failure modes were never shared with sonner's: no channels flag, no registration, no roster expiry, no inbound-deaf sessions.
